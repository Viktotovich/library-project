...
function addBook(e) {
  ...
  if (formValidator.validateAll(e)) {
...
const formValidator = {
  ...
  initiateValidators: function () {
    this.bookTitle.addEventListener("input", this.validateBookTitle);
    this.bookAuthor.addEventListener("input", this.validateBookAuthor);
    this.bookYear.addEventListener("input", this.validateBookYear);
    this.bookPages.addEventListener("input", this.validateBookPages);
  },
  validateBookTitle: function () {
    if (formValidator.bookTitle.validity.tooLong) {
      formValidator.bookTitle.setCustomValidity(
  ...
  validateBookAuthor: function () {
    if (formValidator.bookAuthor.validity.tooLong) {
      formValidator.bookAuthor.setCustomValidity(
        "Please provide a shorter author name"
      );
    } else if (formValidator.bookAuthor.validity.tooShort) {
      formValidator.bookAuthor.setCustomValidity(
        "Please provide a longer author name"
      );
    } else {
      return true;
    }
  },
  validateBookYear: function () {
    if (formValidator.bookYear.validity.rangeOverflow) {
      formValidator.bookYear.setCustomValidity(
        "Please provide a valid book publishment date"
      );
    } else {
      return true;
    }
  },
  validateBookPages: function () {
    if (formValidator.bookPages.validity.rangeOverflow) {
      formValidator.bookPages.setCustomValidity(
        "Please provide something within the range of 2000 pages"
      );
    } else if (formValidator.bookPages.validity.rangeUnderflow) {
      formValidator.bookPages.setCustomValidity(
        "Please provide a valid page range"
      );
    } else {
      return true;
    }
  ...
  validateAll: function (e) {
    e.preventDefault();
    if (
      formValidator.validateBookTitle() &&
      formValidator.validateBookAuthor() &&
      formValidator.validateBookYear() &&
      formValidator.validateBookPages()
    ) {
      //send to server
      return true;
    } else {
...
renderLibrary();
formValidator.initiateValidators();
